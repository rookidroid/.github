...
function rd_shop_init(): void {
    ...
    add_shortcode( 'rookidroid_shop_page',           'rd_shop_page_shortcode' );
    add_action( 'wp_ajax_rd_filter_products',        'rd_shop_ajax_filter' );
    add_action( 'wp_ajax_nopriv_rd_filter_products', 'rd_shop_ajax_filter' );
}
...
function rd_shop_enqueue_assets(): void {
    ...
    wp_localize_script( 'rd-shop', 'rdShop', [
        ...
        'filterNonce' => wp_create_nonce( 'rd_filter_nonce' ),
...
// ── Sort arg mapper ───────────────────────────────────────────────────────────
function rd_shop_map_sort_to_args( string $sort ): array {
    switch ( $sort ) {
        ...
        default:           return [ 'orderby' => 'menu_order', 'order' => 'ASC'  ];
    }
}

// ── [rookidroid_shop_page] shortcode ─────────────────────────────────────────
/**
 * Renders a full shop page layout: filter bar, category/price sidebar, and product grid.
 * Designed for use on the /shop/ page in WordPress with Neve.
 *
 * Usage:
 *   [rookidroid_shop_page]
 *   [rookidroid_shop_page categories="3d-model,electronics,source-code,gadget"
 *       labels="3D Models,Electronics,Software,Gadgets"
 *       columns="3" limit="12"]
 *
 * Attributes:
 *   categories    string  Comma-separated category slugs (default: 3d-model,electronics,source-code,gadget)
 *   labels        string  Comma-separated display labels matching categories
 *   columns       int     Grid columns 1–4 (default: 3)
 *   limit         int     Products per page (default: 12)
 *   orderby       string  Initial sort (default: date)
 *   order         string  ASC|DESC (default: DESC)
 *   show_sidebar  bool    Show sidebar (default: true)
 *   grid_id       string  Product grid DOM ID (default: productGrid)
 */
function rd_shop_page_shortcode( array $atts ): string {
    $atts = shortcode_atts( [
        'categories'   => '3d-model,electronics,source-code,gadget',
        'labels'       => '3D Models,Electronics,Software,Gadgets',
        'columns'      => 3,
    ...
    // Total count across all configured categories
    $total_args            = rd_shop_build_query_args( [
        'category' => implode( ',', $cat_slugs ),
        'limit'    => -1,
        'orderby'  => 'date',
        'order'    => 'DESC',
        'ids'      => '',
    ...
    // Initial product grid (server-side rendered)
    $init_query = new WP_Query( rd_shop_build_query_args( [
        'category' => implode( ',', $cat_slugs ),
        'limit'    => $limit,
        'orderby'  => $atts['orderby'],
        'order'    => $atts['order'],
    ...
    $cards = '';
    foreach ( $init_query->posts as $post ) {
        $product = wc_get_product( $post->ID );
        if ( $product ) {
            $cards .= rd_shop_render_shop_card( $product );
        }
    ...
    $chip_html  = '<button class="chip is-active" data-filter="all">'
                . esc_html__( 'All', 'rookidroid-shop' ) . '</button>';
    foreach ( $cat_slugs as $i => $slug ) {
        $lbl        = $cat_labels[ $i ] ?? ucfirst( str_replace( '-', ' ', $slug ) );
        $chip_html .= '<button class="chip" data-filter="' . esc_attr( $slug ) . '">'
                    . esc_html( $lbl ) . '</button>';
    }

    // Sidebar: category buttons
    $sb_cats  = '<button class="rd-sidebar-btn is-active" data-filter="all">'
              . esc_html__( 'All Products', 'rookidroid-shop' )
              . ' <span>' . esc_html( (string) $total_count ) . '</span></button>';
    foreach ( $cat_slugs as $i => $slug ) {
        $lbl     = $cat_labels[ $i ] ?? ucfirst( str_replace( '-', ' ', $slug ) );
        $cnt     = $term_counts[ $slug ] ?? 0;
        $sb_cats .= '<button class="rd-sidebar-btn" data-filter="' . esc_attr( $slug ) . '">'
                  . esc_html( $lbl )
                  . ' <span>' . esc_html( (string) $cnt ) . '</span></button>';
    }

    // Sidebar: price buttons
    $price_ranges = [
        [ 'value' => 'all',     'label' => __( 'All Prices',  'rookidroid-shop' ) ],
        [ 'value' => '0-0',     'label' => __( 'Free',        'rookidroid-shop' ) ],
        [ 'value' => '1-15',    'label' => __( 'Under $15',   'rookidroid-shop' ) ],
        [ 'value' => '15-30',   'label' => __( '$15 – $30',   'rookidroid-shop' ) ],
        [ 'value' => '30-9999', 'label' => __( '$30 & Above', 'rookidroid-shop' ) ],
    ];
    $sb_prices = '';
    foreach ( $price_ranges as $idx => $rng ) {
        $sb_prices .= '<button class="rd-sidebar-btn' . ( $idx === 0 ? ' is-active' : '' ) . '"'
                    . ' data-price="' . esc_attr( $rng['value'] ) . '">'
                    . esc_html( $rng['label'] ) . '</button>';
    }

    $showing_text = sprintf(
        /* translators: %d: number of products shown */
        _n( 'Showing %d product', 'Showing %d products', $total_count, 'rookidroid-shop' ),
        $total_count
    );
    ...
    if ( $show_sidebar ) {
        $sidebar_html = sprintf(
            '<aside class="rd-shop-sidebar">
                <div class="rd-sidebar-section">
                    <h3 class="rd-sidebar-section__title">%s</h3>
                    <div id="sidebarCategoryFilter">%s</div>
                </div>
                <div class="rd-sidebar-section">
                    <h3 class="rd-sidebar-section__title">%s</h3>
                    <div id="sidebarPriceFilter">%s</div>
                </div>
            </aside>',
            esc_html__( 'Categories', 'rookidroid-shop' ),
            $sb_cats,
            esc_html__( 'Price Range', 'rookidroid-shop' ),
            $sb_prices
        );
    }

    return sprintf(
        '<div class="rd-shop-page" id="rdShopPage">
            <div class="rd-shop-filterbar">
                <div class="rd-shop-search">
                    <svg class="rd-shop-search__icon" width="15" height="15" viewBox="0 0 24 24"
                ...
                <div class="rd-chip-group" id="chipCategoryFilter">%s</div>
                <select id="sortSelect" class="rd-sort-select" aria-label="%s">
                    <option value="featured">%s</option>
                    <option value="newest">%s</option>
                    <option value="price-asc">%s</option>
                    <option value="price-desc">%s</option>
                    <option value="name-asc">%s</option>
                    <option value="popularity">%s</option>
                </select>
            </div>
            <div class="rd-shop-main%s">
                %s
                <div class="rd-shop-content">
                    <div class="rd-shop-result-bar">
                        <span id="resultCount">%s</span>
                    </div>
                    <div class="product-grid rd-shop-grid rd-shop-grid--cols-%d"
                         id="%s" data-limit="%d" data-columns="%d">%s</div>
                    <div class="rd-pagination" id="shopPagination"></div>
                </div>
            </div>
        </div>',
        esc_attr__( 'Search products…', 'rookidroid-shop' ),
        $chip_html,
        esc_attr__( 'Sort products', 'rookidroid-shop' ),
        esc_html__( 'Featured',          'rookidroid-shop' ),
        esc_html__( 'Newest',            'rookidroid-shop' ),
        esc_html__( 'Price: Low → High', 'rookidroid-shop' ),
        esc_html__( 'Price: High → Low', 'rookidroid-shop' ),
        esc_html__( 'Name A–Z',          'rookidroid-shop' ),
        esc_html__( 'Popularity',        'rookidroid-shop' ),
        $show_sidebar ? '' : ' rd-shop-main--no-sidebar',
        $sidebar_html,
        esc_html( $showing_text ),
        $cols,
        esc_attr( $grid_id ),
        $limit,
        $cols,
...
// ── AJAX: filter / sort / search / paginate products ─────────────────────────
function rd_shop_ajax_filter(): void {
    check_ajax_referer( 'rd_filter_nonce', 'nonce' );

    $category    = sanitize_key( wp_unslash( $_POST['category']    ?? '' ) );
    $search      = sanitize_text_field( wp_unslash( $_POST['search']      ?? '' ) );
    $sort        = sanitize_key( wp_unslash( $_POST['sort']        ?? 'featured' ) );
    $price_range = sanitize_text_field( wp_unslash( $_POST['price_range'] ?? 'all' ) );
    $page        = max( 1, absint( $_POST['page']    ?? 1 ) );
    $limit       = min( 48, max( 1, absint( $_POST['limit']   ?? 12 ) ) );
    $cols        = min( 4,  max( 1, absint( $_POST['columns'] ?? 3  ) ) );

    $sort_args  = rd_shop_map_sort_to_args( $sort );
    $query_args = rd_shop_build_query_args( [
        'category' => $category,
        'limit'    => $limit,
    ...
    if ( $price_range && 'all' !== $price_range ) {
        $parts = array_map( 'floatval', explode( '-', $price_range ) );
        if ( count( $parts ) === 2 ) {
            if ( 0.0 === $parts[0] && 0.0 === $parts[1] ) {
                // Free products
